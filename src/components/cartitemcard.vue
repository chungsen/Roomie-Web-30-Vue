<template>
  <div class="cart.item.card cart.card.align.center" v-bind:class="rootClassName">
    <div class="card.item.img">
      <img :alt="image_alt" :src="itemimg" class="level2.category.img" />
    ...
    <div class="cart.item.card.text">
      <div class="cart.title.group">
        <span class="cart.item.card-text item.list.card.price">{{ title }}</span>
        <span class="cart.card.price.margin item.list.card.price">
          {{ price }}
        </span>
      </div>
      <span class="cart.item.desc">{{ cartitemdesc }}</span>
    </div>
  </div>
</template>
